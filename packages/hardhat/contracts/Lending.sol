...
contract Lending is Ownable {
    ...
    function _calculatePositionRatio(address user) internal view returns (uint256) {
        ...
        return (calculateCollateralValue(user) * 1 ether) / borrowedAmount;
    }
    ...
    function isLiquidatable(address user) public view returns (bool) {
        return _calculatePositionRatio(user) * 100 < COLLATERAL_RATIO * 1 ether;
    }
    ...
    function liquidate(address user) public {
        ...
        if (i_corn.balanceOf(msg.sender) < debt) revert Lending__InsufficientLiquidatorCorn();

        bool success = i_corn.transferFrom(msg.sender, address(this), debt);
        if (!success) revert Lending__RepayingFailed();

        uint256 price = i_cornDEX.currentPrice();
        uint256 collateralForDebt = (debt * 1 ether) / price;
        uint256 collateralForLiquidator = (collateralForDebt * (100 + LIQUIDATOR_REWARD)) / 100;
        uint256 userCollateral = s_userCollateral[user];
        if (collateralForLiquidator > userCollateral) {
            collateralForLiquidator = userCollateral;
        }

        s_userBorrowed[user] = 0;
        s_userCollateral[user] = userCollateral - collateralForLiquidator;

        (success, ) = payable(msg.sender).call{ value: collateralForLiquidator }("");
        if (!success) revert Lending__TransferFailed();

        emit Liquidation(user, msg.sender, collateralForLiquidator, debt, price);
    }
}
